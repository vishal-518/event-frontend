    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await fetch(
          "https://event-backend-cg59.onrender.com/api/events",
          {
            headers: {
        ...
        // ❌ If backend returns error object → map crash fix
        if (!response.ok || !Array.isArray(data)) {
          console.log("Invalid events response:", data);
          setEvents([]); // Safe
          setLoading(false);
          return;
        }
      ...
      {events.length === 0 ? (
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {events.map((event) => (
            <div
              key={event._id}
              className="bg-gray-800 rounded-xl shadow-xl overflow-hidden border border-gray-700/40 hover:border-blue-500/50 hover:shadow-blue-500/30 transition duration-300"
            >
              {/* Image */}
              <img
                src={
              ...
              <div className="p-5">
                ...
                {/* Status Badge */}
                <span
                  className={`inline-block mt-4 px-3 py-1 rounded-full text-xs font-medium ${getStatusStyle(
                    event.status || "pending"
                  )}`}
                >
                  {event.status ? event.status.toUpperCase() : "PENDING"}
